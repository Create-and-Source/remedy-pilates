...
const COLLECTIONS = [
  ...
  { id: "COL-4", name: "Kelly's Favorites", description: "Hand-picked by our founder for the ultimate Remedy experience", classIds: ["VOD-001", "VOD-012", "VOD-005", "VOD-019"], icon: "\u2B50", color: "#A68B6B" },
];
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={onClose}>
      <div style={{ ...s.card, padding: 32, maxWidth: 720, width: "90%", background: "#fff" }} onClick={e => e.stopPropagation()}>
        <div style={{ textAlign: "center", marginBottom: 28 }}><div style={s.label}>Remedy On-Demand</div><h2 style={{ fontFamily: DISPLAY, fontSize: 28, color: "#111", margin: "8px 0 4px" }}>Your studio, anywhere</h2><p style={{ fontFamily: FONT, fontSize: 14, color: "#777", margin: 0 }}>Stream every class from all 3 Remedy locations, on any device.</p></div>
...
export default function OnDemandLibrary() {
  ...
  if (view === "player" && selectedClass) {
    ...
    return (<div style={{ fontFamily: FONT, minHeight: "100vh", background: "#FAF6F1", padding: "24px 32px" }}><VideoPlayerView classData={selectedClass} instructor={instructor} onBack={() => { setView("library"); setSelectedClass(null); }} allClasses={classes} /></div>);
  }
  ...
  return (
    <div style={{ fontFamily: FONT, minHeight: "100vh", background: "#FAF6F1" }}>
      <div style={{ padding: "24px 32px", maxWidth: 1280, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 28 }}><div style={s.label}>Featured This Week</div><div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginTop: 10 }}>{featured.map(cls => { const ins = INSTRUCTORS.find(i => i.id === cls.instructorId); return (<div key={cls.id} onClick={() => openClass(cls)} style={{ ...s.card, cursor: "pointer", overflow: "hidden", transition: "transform 0.2s, box-shadow 0.2s" }} onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-2px)"; e.currentTarget.style.boxShadow = "0 8px 32px rgba(0,0,0,0.08)"; }} onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = s.card.boxShadow; }}><ClassThumbnail classData={cls} instructor={ins} size="medium" /><div style={{ padding: "12px 14px" }}><div style={{ fontFamily: FONT, fontSize: 14, fontWeight: 600, color: "#111", marginBottom: 4, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{cls.title}</div><div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ fontFamily: FONT, fontSize: 12, color: "#999" }}>{ins?.name.split(" ")[0]}</span><span style={{ display: "flex", alignItems: "center", gap: 3, fontFamily: FONT, fontSize: 12, color: "#999" }}><StarIcon size={11} /> {cls.rating}</span></div></div></div>); })}</div></div>

        <div style={{ marginBottom: 28 }}><div style={s.label}>Collections</div><div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginTop: 10 }}>{COLLECTIONS.map(col => (<button key={col.id} onClick={() => setActiveCollection(activeCollection === col.id ? null : col.id)} style={{ ...s.card, padding: "14px 16px", cursor: "pointer", textAlign: "left", border: activeCollection === col.id ? `2px solid ${col.color}` : `1px solid ${CARD_BORDER}`, background: activeCollection === col.id ? `${col.color}08` : CARD_BG, transition: "all 0.2s" }}><div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}><span style={{ fontSize: 18 }}>{col.icon}</span><span style={{ fontFamily: FONT, fontSize: 13, fontWeight: 600, color: "#111" }}>{col.name}</span></div><div style={{ fontFamily: FONT, fontSize: 11, color: "#999" }}>{col.classIds.length} classes</div></button>))}</div></div>
        ...
        {layout === "grid" && (<div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 18 }}>{filtered.map(cls => { const ins = INSTRUCTORS.find(i => i.id === cls.instructorId); return (<div key={cls.id} onClick={() => openClass(cls)} style={{ ...s.card, cursor: "pointer", overflow: "hidden", transition: "transform 0.2s, box-shadow 0.2s" }} onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-3px)"; e.currentTarget.style.boxShadow = "0 12px 40px rgba(0,0,0,0.1)"; }} onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = s.card.boxShadow; }}><ClassThumbnail classData={cls} instructor={ins} /><div style={{ padding: "14px 16px" }}><div style={{ fontFamily: FONT, fontSize: 15, fontWeight: 600, color: "#111", marginBottom: 4 }}>{cls.title}</div><div style={{ fontFamily: FONT, fontSize: 12, color: "#777", marginBottom: 10, lineHeight: 1.5, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{cls.description}</div><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}><div style={{ display: "flex", alignItems: "center", gap: 8 }}><div style={{ width: 22, height: 22, borderRadius: "50%", background: ins?.color || ACCENT, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontFamily: FONT, fontSize: 8, fontWeight: 700 }}>{ins?.name.split(" ").map(n => n[0]).join("")}</div><span style={{ fontFamily: FONT, fontSize: 12, color: "#555" }}>{ins?.name}</span></div><div style={{ display: "flex", alignItems: "center", gap: 8, fontFamily: FONT, fontSize: 12, color: "#999" }}><span style={{ display: "flex", alignItems: "center", gap: 3 }}><StarIcon size={11} /> {cls.rating}</span><span>{cls.views.toLocaleString()} views</span></div></div></div></div>); })}</div>)}
